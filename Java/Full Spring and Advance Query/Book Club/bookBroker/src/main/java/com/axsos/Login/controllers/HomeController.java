...
//	-------------chooseBookPage-------------------
	...
	public String runderEditBookPage(@PathVariable("id") Long id, Model model, HttpSession session) {
		...
		Book book = bookSer.findBook(user_id);
	...
	public String EditBook(@PathVariable("id") Long id,@Valid @ModelAttribute(value = "book") Book book, BindingResult result, HttpSession session) {
		if (result.hasErrors()) {
			return "editBook.jsp";
		} else {
			Long user_id = (Long) session.getAttribute("user_id");
...
//			bookSer.updateBook(book);
			bookSer.updateBookAdvance(id, book);
			return "redirect:/home";
		}
	}
	...
	public String bookmarket(HttpSession session, Model model) {
		...
		List<Book> books = bookSer.unborrowerBooks(userServ.findUserById(userId));
		model.addAttribute("unborrowedBooks", books);

		List<Book> myBooks = bookSer.borrowerBooks(userServ.findUserById(userId));
		model.addAttribute("borrowedBooks", myBooks);
